...
- epoched proof-of-stake, where changes are applied as follows:
  ...
  - slashing is applied as described in [slashing](cubic-slashing.mdx).
...
While rewards are given out at the end of every epoch, voting power is only updated after the pipeline offset. According to the [proof-of-stake system](objects-and-txs.mdx#epoched-data),  at the current epoch `e`, the validator sets can only be updated for epoch `e + pipeline_offset`, and it should remain unchanged from epoch `e` to `e + pipeline_offset - 1`. Updating voting power in the current epoch would violate this rule.
