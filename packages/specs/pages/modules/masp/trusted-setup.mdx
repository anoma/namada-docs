...
2. A [HTTP REST API](https://github.com/anoma/namada-trusted-setup/blob/9c3e6c63c0279b0dc9b1ab5ce698548ee8ada766/phase2-coordinator/src/rest.rs) that interfaces with the CL.
3. A [CLI implementation](https://github.com/anoma/namada-trusted-setup/tree/main/phase2-cli) that communicates with the HTTP REST API endpoints.
...
To be able to reuse the CL without heavy refactoring, we decided to keep most of the Aleo code as it is and only change the parts that needed to be changed, more precisely the crypto functions (`initialize`, `contribute` and `verify`) and the coordinator config `environment.rs`.

## 2. HTTP REST API

### Description

The [HTTP REST API](https://github.com/anoma/namada-trusted-setup/blob/9c3e6c63c0279b0dc9b1ab5ce698548ee8ada766/phase2-coordinator/src/rest.rs) is a rocket web server that interfaces with the CL. All requests need to be signed to be accepted by the endpoints. It's the core of the ceremony where the Coordinator is started together with utility functions like `verify_contributions`  and `update_coordinator`.
...
![](/images/trusted-setup-assets/namada-ts-swimlane.png)
