...
The validator's voting-power is proportional to the sum of its self-bonded tokens and all the bonded tokens from other addesses to their own address.
...
Anyone holding some `NAM` will be able to propose some changes in a proposal for which delegators and validators will cast their `yay` or `nay` votes. 
It will also be possible to attach some payloads to proposals, in specific cases, to embed additional information. 
...
1. [**On-chain**](#on-chain-governance) - A proposal is submitted to the Namada blockchain, and the Namada blockchain will handle the voting process.
