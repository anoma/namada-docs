...
To integrate a sapling-like circuit (as MASP) into Namada we have to take the differences between the execution models
between Namada and Zcash, for which sapling was originally specified.
AS Zcash does not follow exactly a UTXO model, due to the presence of Notes in architecture. However, the spending of a
...
### Note Format
The note structure needs to encode an asset's type, its quantity and its owner. In our implementation has the following format:
```
struct Note {
