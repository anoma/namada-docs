The name of the chapter is "Ledger integration" and it contains three main paragraphs:
* MASP integration spec
* Shielded Transfer Specification
* Storage Interface Specification

It would be useful here to have a more clear description of what is taken from the zcash
sapling circuits and what is new, as it's done in "Multi-Asset Shielded Pool Specification Differences from Zcash
Protocol Specification", but have it more
...
1. "following the MASP spec" is tautological here, as this is the MASP spec itself.
...
1. Shield value pool was not introduced yet. It would be good to have references here at what a shielded value pool is, and what it does.
Here we use interchangeably shielded pool and Multi Asset Shielded Pool.
...
In future¹, the shielded pool will be able to update the note commitment and
...
1. What is happening now, or in the immediate future?
...
# Shielded Transfer Specification
It appears to me that there is no difference between  shielded transfer and a shielded transaction. It would be better
to pick one of the two names and stick with it
...
* Otherwise the shielded component must have the form `Some(x)` where `x` has the transaction encoding specified in the [Multi-Asset Shielded Pool Specs]() (which is this very document? Link missing)
...
into the shielded pool, and allowed conversions between various asset types. Below we describe the ABCI paths and
the encodings of the responses to each type of query.
...
1. How is the count query related to the stopping of the scanning?
