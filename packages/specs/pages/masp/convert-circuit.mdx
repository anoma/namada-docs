...
The incentive system provides a mechanism in which the old asset (input) is burned, the new asset (output) is minted with the same quantity and incentive asset (reward) is minted with the convert ratio meanwhile.
...
1. There should not be a question here, are they or aren't they mandatory?
