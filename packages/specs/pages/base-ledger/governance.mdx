...
- A Namada _validator_ is an account with a public consensus key, which may participate in producing blocks and governance activities. A validator may not also be a delegator (although, of course, a user can control both validator and delegator keys).
