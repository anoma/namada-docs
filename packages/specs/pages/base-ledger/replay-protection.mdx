...
The Namada protocol relies on the CometBFT substrate to exchange transactions (messages)
...
{/* Where is the metadata in the WrapperTx struct? */}
...
2. The hash of this transaction gets signed by the author, producing another
   ...
   {/* Do we have two Tx for each transaction? */}
...
4. Finally, the `WrapperTx` is converted to a `Tx` struct, signed over its
   ...
   network
   {/* Do we have three Tx (or 4, since step 2 is repeated here) for each transaction? Do they all get submitted? */}
...
1. The `WrapperTx` signature is verified and, if valid, the tx is processed
...
{/* I don't think execution.md provides a more in-depth view */}
{/* What prevents a malicious user to submit an already signed transaction? I.e. to replay it? */}
{/* Don't we need a nonce to avoid double spending? */}
...
{/* Weren't we explaining the implementation earlier? */}
...
{/* "The modification also..." which modification? */}
...
...
```
{/* I don't understand this. Why is a subspace in storage required? */}
