...
{/* Why do we want multisignatures? */}
{/* What are encrypted (inner) WASM transactions? What other transactions are there? What is inner? Why are we only targeting encrypted (inner) WASM transactions in this k-of-n multisignatures document? */}
{/* Why is not supporting multiple signers important? */}
{/* What are wrapper/protocol transactions?  */}

## Protocol

Namada transactions are signed before being delivered to the network. This signature is checked by the invoked validity predicates to determine the validity of the transaction. To support multisignature, Namada's signed transaction data include the plaintext data of what is being signed, as well as all valid signatures over that data.
{/* Who signs a transaction? */}
{/* Which validity predicates check the signature? All of them? */}
...
{/* How does the algorithm look like? Is the execution complexity reduced because we are using this key-value table instead of a list, or there is something else? */}
...
{/* What is a user-owner account? */}
{/* Suggestion: reference the vp_user validity predicate */}
...
{/* Does this apply to only this vp_user or to all vps? */}
{/* Is this the only information a validity predicate requires? */}
...
```
{/* Where does this storage live? I'd like to see a wider picture. Is there only one storage or multiple storages? If many, how does this storage relate to other storages? */}
...
{/* Can we point to the file where this is done? */}
...
{/* What is an established address? What other types of addresses are there? */}
...
{/* Does the storage writes modify the vp_user address, the threshold or the list of public keys? This sounds weird */}
...
{/* What are the consequences of providing incorrect data? What kind of data are we talking about? */}
...
{/* Do we want to leave these different non-implemented options here? */}
...
## Multisig transaction construction
