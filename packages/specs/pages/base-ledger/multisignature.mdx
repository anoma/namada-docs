...
2. Enough **valid** signatures (out of these unique ones) for the given threshold are provided

Say we have a threshold at 4 and we are provided with 15 signature. 10 of them are unique but not valid, 5 of them are valid but are not unique. Would the VP verify the correctness of the signature as valid?
