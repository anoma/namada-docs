...
{/* What is the Anoma execution model? Link?  */}
{/* How is Namada execution model different? Maybe list the differences */}
{/* I think this introduction needs to be more elaborate. What is an execution model? What are the main criteria for choosing Namada execution model? */}
{/* Reference IBC and MASP */}
...
{/* What is the accounts' dynamic storage sub-space? Is it a Merkle tree? Is it a key value store? What are its properties?  */}
...
{/* Why do we want DKG and threshold decryption integration? I would elaborate on the motivation and consequences of executing transactions in the Endblock method */}
...
{/* "in which UTXO-based systems such as the MASP can be internally implemented as specific accounts": How is this done? When we say "can be implemented", does it mean they can the be implemented differently? */}
{/* How does a validity predicate govern an account? Is the address a hash of a validity predicate? How does Namada map this relationship? */}
...
{/* "A transaction may also explicitly elect an account as the verifier of that transaction, which will result in that validity predicate being invoked as well" Why would a transaction elect an account as the verifier? How does the transaction do this? Who decides? */}
...
{/* What is the reason and the consequences? What are the limitations? */}
