...
Another important note is that the voting period differs between `delegates` and `delegators`. The `delegates` have a shorter voting period than the `delegators`. This is to ensure that the `delegates` have enough time to vote on the proposals. See the specs for more information.
</Callout>