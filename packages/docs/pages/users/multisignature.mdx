...
<Callout type="info">
The `--signing-keys` or the `--fee-payer` argument is still required, despite the fact that the transaction is offline. This just has an effect on who will pay the fee once it is submitted.
</Callout>
