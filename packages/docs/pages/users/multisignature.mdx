...
<Callout type="info">
The `--signing-keys` or the `--gas-payer` argument is still required, despite the fact that the transaction is offline. This just has an effect on who will pay the fee once it is submitted.
</Callout>

The executed command gives some output similar to the following:
```md
...
This executed command will give some output similar to the following:
```md
