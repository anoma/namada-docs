...
For `v0.21.1` there are certain limitations to the offline transaction construction. Please be weary of any bugs that may arise.
</Callout>
...
Within the specified folder, a `.tx` file will be created. This file contains the hexidecimal representation of the transaction bytes. This file can be used to sign the transaction.
...
Also note teh `tx_dumps` folder. This is the folder where the transaction was dumped to as specified in `--output-folder-path` in the previous step.
</Callout>
