...
Before making an unshielding transfer, it is recommended to first sync the local shielded wallet.
See how to do that [here](shielded-sync.mdx).

To make an unshielding transfer, you must possess the relevant spending key. See the [introduction to Namada keys and addresses](../addresses.mdx#shielded-accounts) for
