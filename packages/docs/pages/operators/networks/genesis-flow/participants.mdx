...
By convention, a directory for each pre-genesis network participant is created in the github repository. The `signed-transactions.toml` file is then submitted to the respective directory.
