...
```bash copy
export COMMIT="6c6bdfe26c1ccfeb168b73c55421a1de9d7efc1c"
...
### Set up nodes to join existing Namada instances
The script `join-namada.sh` will set up two nodes for two instances, copy necessary files for Hermes, and make an account for Hermes on each ledger. Also, it will make a Hermes' config file `config_for_namada.toml` in the `hermes` directory.
...
The script `setup-namada.sh` will set up two instances with one validator node, copy necessary files for Hermes, and make an account for Hermes on each ledger. Also, it will make a Hermes' config file `config_for_namada.toml` in the `hermes` directory.
```bash copy
git clone git@github.com:heliaxdev/hermes.git
git checkout $COMMIT # The branch is the same as our Hermes
cd hermes
