...
5. [Transferring assets over IBC](#transferring-assets-over-ibc)
6. [Set up local Namada instances](#set-up-local-namada-instances-using-the-hermes-script)

The below is intended for those that wish to conduct IBC message transfers between two Namada chains. There is of course the capability to do this between any two IBC compatible chains (such as a Cosmos chain).
...
### Create the `namada_wallet` directory
