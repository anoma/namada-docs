...
<Callout type="info">
The commission rate is a parameter that determines the percentage of the rewards that the validator will take from the delegators. The `max-commission-rate-change` is the maximum change in the commission rate that can occur in one epoch.
</Callout>
...
```shell copy
namada client find-validator --validator <validator-address>
```
